...
# 2. Setup Persona and Instance
persona = "You are a college professor, your answer always starts with: Dear students,"
# Using a slightly larger chunk size to reduce API calls (prevents rate-limit crashes)
rag_agent = RAGKnowledgePromptAgent(openai_api_key, persona, 1000, 100)
...
# 3. The Critical Step: Assigning and Processing
print("--- [DEBUG] Starting Chunking ---")
chunks = rag_agent.chunk_text(knowledge_text)

print("--- [DEBUG] Calculating Embeddings (API Call) ---")
# Spelling fixed to 'embeddings'
embeddings = rag_agent.calculate_embeddings()

# A 1-second pause ensures the CSV is fully written to disk by the OS
time.sleep(1)
...
try:
    # Use the method defined in your class
