...
class VectorStoreManager:
    ...
    def __init__(self, openai_api_key: Optional[str] = None, persist_directory: Optional[str] = None):
    ...
    def _create_embedding_function(self, api_key: Optional[str]) -> EmbeddingFunction:
        if api_key:
    ...
    def create_store(self, store_name: str, force: bool = False) -> VectorStore:
        ...
        try:
            ...
            raise ValueError(
                f"Collection '{store_name}' already exists. Pass force=True or use get_or_create_store()."
            ) from exc
